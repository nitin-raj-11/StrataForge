...
export default function EquityChart({ values, multiplier = 1, currency = "USD" }: { values: number[]; multiplier?: number; currency?: string }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(ref.current, { width: ref.current.clientWidth, height: 280, layout: { background: { color: bgColor }, textColor }, grid: { vertLines: { color: gridColor }, horzLines: { color: gridColor } }, localization: { priceFormatter: (value) => `${currency === "INR" ? "₹" : "$"}${value.toFixed(0)}` } });
